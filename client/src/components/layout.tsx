...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const isTransparentPage = location === "/" || location === "/services/color";
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-8 text-center md:text-left">
            <div className="flex flex-col gap-4">
              ...
              <div className="flex gap-4 justify-center md:justify-start">
                {/* Social placeholders */}
                <a href="https://www.instagram.com/yuny_beauty" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-gray-200 flex items-center justify-center text-foreground hover:bg-primary hover:text-white transition-colors cursor-pointer">
            ...
            <div className="flex flex-col gap-2 text-sm font-medium text-primary">
              <p>{t.footerAddress}</p>
              <p>{(t as any).footerPhone || "786-436-8830"}</p>
              <p>{t.footerHours}</p>
            </div>
          </div>
